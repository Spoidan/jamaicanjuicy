        ...
        <div className="flex flex-col flex-1 p-4 sm:p-5 gap-3">
          ...
          <div>
            <div className="flex items-start justify-between gap-2 mb-1">
              ...
              {/* Clickable rating → opens reviews drawer */}
              <button
                onClick={() => setReviewsOpen(true)}
                className="flex items-center gap-1 flex-shrink-0 hover:opacity-70 transition-opacity group/rating"
                aria-label={`${product.reviewCount} reviews — click to read`}
              >
                <Star className="w-3.5 h-3.5 fill-mango text-mango" />
                <span className="text-xs font-semibold">{product.rating}</span>
                <span className="text-xs text-neutral-400 group-hover/rating:underline">({product.reviewCount})</span>
              </button>
            </div>
